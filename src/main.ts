...
export default class Hackatime extends Plugin {
  ...
  private _sendHeartbeat(
    ...
    const user_agent = 'obsidian/' + apiVersion + ' hackatime-obsidian/' + this.manifest.version;
        ...
        if (this.showStatusBar) {
          if (output && output.trim()) {
            if (this.showCodingActivity) {
              ...
              this.updateStatusBarTooltip('Hackatime: Today’s coding time.');
            } else {
              this.updateStatusBarText();
            ...
            this.updateStatusBarText();
            this.updateStatusBarTooltip('Hackatime: Calculating time spent today in background...');
          }
        }
      } else if (code == 102 || code == 112) {
