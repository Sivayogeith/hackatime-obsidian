...
import { HACKATIME_URL, LogLevel } from "./constants";
import { Utils } from "./utils";
import { Dependencies } from "./dependencies";
import { Desktop } from "./desktop";

export default class Hackatime extends Plugin {
  options: Options;
  ...
  public initializeDependencies(): void {
    this.logger.debug(`Initializing Hackatime v${this.manifest.version}`);
  ...
  public promptForProjectName(): void {
    new ProjectNameModal(this.app, this.options).open();
  }
  ...
  private _sendHeartbeat(
    ...
    this.options.getSetting(
      ...
      (setting: OptionSetting) => {
        ...
        args.push(
          "--project",
          String(setting.value || this.app.vault.getName())
        );
        args.push("--api-url", String(HACKATIME_URL));
  ...
  private _getCodingActivity() {
    ...
    this.options.getSetting(
      ...
      (setting: OptionSetting) => {
        ...
        const binary = this.dependencies.getCliLocation();
        this.logger.debug(
          `Fetching coding activity for Today from api: ${Utils.formatArguments(
            binary,
            args
          )}`
        );
  ...
  onOpen() {
    ...
    this.options.getSetting(
      ...
      (setting: OptionSetting) => {
        ...
        contentEl.createEl("h2", { text: "Enter your Hackatime API Key" });
...
class ProjectNameModal extends Modal {
  options: Options;
  input: TextComponent;
  private static instance: ProjectNameModal;

  constructor(app: App, options: Options) {
    if (ProjectNameModal.instance) {
      return ProjectNameModal.instance;
    }
    super(app);
    this.options = options;
    ProjectNameModal.instance = this;
  }
  ...
  onOpen() {
    ...
    this.options.getSetting(
      ...
      (setting: OptionSetting) => {
        ...
        contentEl.createEl("h2", { text: "Enter your Project Name" });
